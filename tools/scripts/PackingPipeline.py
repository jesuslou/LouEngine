...
class PackGenerator:
	...
	def generate_pack_definitions(self):
		...
		for pack_definition in self.packs_info["packs"]:
			if self.__any_file_modified(pack_definition):
				...
				self.packs_needing_regeneration.append(pack_definition["packName"])
			elif not self.__does_pack_exists_in_res_output_folder(pack_definition):
				self.packs_needing_regeneration.append(pack_definition["packName"])
			else:
	...
	def __does_pack_exists_in_res_output_folder(self, pack_definition):
		packages_with_name = list(self.res_output_path.glob("**/{}.*".format(pack_definition["packName"])))
		return len(packages_with_name) > (1 if pack_definition["separatedPacks"] == True else 0)
	...
	def generate_packs(self):
		if len(self.packs_needing_regeneration) > 0:
			params = []
			...
			params.append(",".join(self.packs_needing_regeneration))
			...
			print("\n-- No Packages were modified. Skipping generation.")


if __name__ == '__main__':
	packs_file = read_parameters()
	packGenerator = PackGenerator(packs_file)
	packGenerator.generate_pack_definitions()
	packGenerator.generate_packs()
	packGenerator.update_cache_files()
