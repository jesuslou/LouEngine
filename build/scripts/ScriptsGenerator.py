...
generate_file_template="""###THIS IS AN AUTOGENERATED FILE: ANY MODIFICATION WILL BE LOST###
#!/bin/bash
...
cmake "{root_cmake_path}" -DCMAKE_CONFIGURATION_TYPES="{configuration}" -DENABLE_TESTS={enable_tests} -DMAX_ENTITIES={max_entities} -DMAX_TAGS={max_tags} {common_cmake_flags} {framework_flags} {platform_flags}

read -p "Press any key to continue..."
"""


update_file_template="""###THIS IS AN AUTOGENERATED FILE: ANY MODIFICATION WILL BE LOST###
#!/bin/bash
...
{python_version} "{script_path}" --folder "{project_folder_path}" --max_entities "{max_entities}" --max_tags "{max_tags}"
...
def generate_templated_file(script_file_path, template, configuration, script_folder_path, root_cmake_path, project_folder_path, common_cmake_flags, extra_flags, platform_flags, max_entities, max_tags):
...
def create_platform_scripts(project_name, deploy_path, platform, max_entities, max_tags):
    ...
    generate_templated_file(generate_script_file_debug, generate_file_template, "Debug", script_folder_platform, root_cmake_path, project_folder_path_debug, common_cmake_flags, extra_flags, platform_flags, max_entities, max_tags)
    generate_templated_file(generate_script_file_release, generate_file_template, "Release", script_folder_platform, root_cmake_path, project_folder_path_release, common_cmake_flags, extra_flags, platform_flags, max_entities, max_tags)

    generate_templated_file(update_script_file_debug, update_file_template, "Debug", script_folder_platform, root_cmake_path, project_folder_path_debug, common_cmake_flags, extra_flags, platform_flags, max_entities, max_tags)
    generate_templated_file(update_script_file_release, update_file_template, "Release", script_folder_platform, root_cmake_path, project_folder_path_release, common_cmake_flags, extra_flags, platform_flags, max_entities, max_tags)


def create_generate_scripts(project_name, deploy_path, platforms, max_entities, max_tags):
    for platform in platforms:
        if platform in VALID_PLATFORMS:
            create_platform_scripts(project_name, deploy_path, platform, max_entities, max_tags)
        else:
            print("Skipping scripts generation for '{}'. Invalid platform! Valid platforms are {}".format(platform, VALID_PLATFORMS))


def create_generator_script(deploy_path, max_entities, max_tags):
